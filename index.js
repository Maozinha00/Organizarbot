 ...
 *  - NENHUMA rotina de autokick ou aviso automático é executada de forma autônoma.
 * ============================================================================
 */
    ...
    if (lowerContent === "!limparcargos" || lowerContent === "!resetgrupos") {
        try {
            ...
            for (const [id, mem] of membros) {
                ...
                // Se o membro já está conectado a um canal de voz (call), não remove os cargos dele
...
if (!TOKEN || TOKEN.trim() === "") {
    console.log("❌ ERRO: O Token do Discord (DISCORD_TOKEN) não foi configurado nas variáveis de ambiente!");
    process.exit(1);
}

client.login(TOKEN).catch(err => {
    console.log("❌ Falha ao logar o bot no Discord. Verifique se o Token é válido:", err);
});
